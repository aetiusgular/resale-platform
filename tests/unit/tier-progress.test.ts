...
describe('resolveEffectiveBps (checkout)', () => {
  it('no stored lock → pure activity rate', async () => {
    const { svc } = fakeService({
      orderRows: Array.from({ length: 15 }, () => ({ item_cents: 200_000 })), // $30k capped, 15 orders → 200
      profile: { current_buyer_tier_bps: null, buyer_tier_locked_until: null },
    })
    expect(await resolveEffectiveBps(svc, 'u', 'buyer' as FeeSide)).toBe(200)
  })
  ...
  it('expired lock → activity rate', async () => {
    ...
    const { svc } = fakeService({
      orderRows: [],
      profile: { current_seller_tier_bps: 200, seller_tier_locked_until: past },
    })
    expect(await resolveEffectiveBps(svc, 'u', 'seller' as FeeSide)).toBe(500)
  })
})
...
describe('applyTierProgress (order completion — rolling lock)', () => {
  ...
  it('upgrade (no active lock) → writes new bps + a ~30-day lock', async () => {
    const { svc, writes } = fakeService({
      orderRows: Array.from({ length: 15 }, () => ({ item_cents: 200_000 })), // → 200
      profile: { current_seller_tier_bps: 500, seller_tier_locked_until: null },
    })
    await applyTierProgress(svc, 'u', 'seller' as FeeSide)
    expect(writes).toHaveLength(1)
    expect(writes[0].current_seller_tier_bps).toBe(200)
  ...
  it('EXPIRED lock with stale better bps + re-earned lower tier → relocks (the #3 fix)', async () => {
    const { svc, writes } = fakeService({
      orderRows: Array.from({ length: 7 }, () => ({ item_cents: 200_000 })), // $1.4M/7 → 350
      profile: { current_seller_tier_bps: 200, seller_tier_locked_until: past() },
    })
    await applyTierProgress(svc, 'u', 'seller' as FeeSide)
    expect(writes).toHaveLength(1)
  ...
  it('first-ever (null bps) → writes current activity + lock', async () => {
    const { svc, writes } = fakeService({
      orderRows: [{ item_cents: 5000 }], // base 500
      profile: { current_buyer_tier_bps: null, buyer_tier_locked_until: null },
    })
    await applyTierProgress(svc, 'u', 'buyer' as FeeSide)
    expect(writes).toHaveLength(1)
    expect(writes[0].current_buyer_tier_bps).toBe(500)
  })
})
