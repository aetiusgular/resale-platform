...
import { isProtoPath, PROTO_BASE } from '@/app/proto/viewer-fixture'
...
describe('proto fixtures', () => {
  ...
  it('treats only /styleguide/proto as the guest tour', () => {
    expect(PROTO_BASE).toBe('/styleguide/proto')
    expect(isProtoPath('/styleguide/proto')).toBe(true)
    expect(isProtoPath('/styleguide/proto/saved')).toBe(true)
    expect(isProtoPath('/browse')).toBe(false)
    expect(isProtoPath('/proto')).toBe(false)
    expect(isProtoPath('/saved')).toBe(false)
  })
})
