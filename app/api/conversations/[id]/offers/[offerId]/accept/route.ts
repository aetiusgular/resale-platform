/**
 * POST /api/conversations/[id]/offers/[offerId]/accept
 * Accept an open offer. Only the RECIPIENT (non-sender) may accept.
...
export async function POST(_req: NextRequest, { params }: RouteContext) {
  ...
  const { data: updated, error } = await service
    ...
    .eq('id', offerId)
    .eq('state', 'open') // guard: prevent double-accept race
    .select()
    .single()
